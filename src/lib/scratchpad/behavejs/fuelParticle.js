import {createConfigFuelSource, createEhn, createSavr} from './templates.js'
import {Savr} from './Variants.js'

import {Quantity} from './baseVariants.js'
function showEdges(variant) {
    console.log(variant.desc, 'has', variant.inputs.size, 'inputs and',
        variant.consumers.size, 'consumers:')
    variant.inputs.forEach((value1, value2, set) =>
        console.log(' - Input:', value1.desc))
    variant.consumers.forEach((value1, value2, set) =>
 ...
 */
console.log('Creating a Variant with Constant Value')
const standardSavr = createSavr('Standard Fuel Model Dead 1-h SAVR', 1234)
...
*/
console.log('\nCreating a Variant whose value depends upon another Variant:')
showEdges(standardSavr)
const standardEhn = createEhn('Standard Fuel Model Dead 1-h EHN', standardSavr)
showEdges(standardSavr)
showEdges(standardEhn)
process.exit()
...
// Second, create the dependent variant passing a {update} prop method as follows:
const primarySavr = createSavr({
    desc: 'Primary Surface Dead 1-h SAVR',
    update: function () {primarySavr.set(getInputSavr())}
})
...
primarySavr.update()
console.log(`- ${primarySavr.desc} updated value is now`, primarySavr.value)
...
primarySavr.update()
console.log(`- ${primarySavr.desc} value now updated to`, primarySavr.value)
 ...
 */
console.log('\nLinking a Variant Value to Another Variant`s Value:')
...
/*******************************************************************************
 * Conditionally linking a variant value to the value of another variant,
 * to client input, or to some computation method.
 * 
 * An example is linking the primary surface fuel dead 1-h SAVR
 * to the value from a standard fuel model OR to client input.
 */
console.log('\nConditionally Linking a Variant Value to Another Variant`s Value or Input:')

// First create a fuel parameter source variant
const configFuel = createConfigFuelSource()
console.log(`${configFuel.desc} is '${configFuel.value}'`)
