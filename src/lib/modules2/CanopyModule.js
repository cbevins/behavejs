 ...
 */
export function defineCanopyModule(parentMod, parentProp) {
    const mod = new DagModule(parentMod, parentProp)

    const crown  = mod.crown = new DagModule(mod, 'crown')
    ...
    const fuel = mod.fuel = new DagModule(mod, 'fuel')
    ...
    const wind = mod.wind = new DagModule(mod, 'wind')
    wind.sheltered = new DagNode(wind, 'sheltered', U.bool)
        .use(Canopy.canopySheltersFuelFromWind,
            [crown.cover, crown.height, crown.fill])
    wind.wsrf = new DagNode(wind, 'wsrf', U.fraction)
        .use(Canopy.canopyWindSpeedAdjustmentFactor,
                    [crown.cover, crown.height, crown.fill])

    return mod
}

export function configCanopyModule(mod, configHeight) {
    const {crown, fuel, wind} = mod
    fuel.bulk.input()
    fuel.heat.input()
