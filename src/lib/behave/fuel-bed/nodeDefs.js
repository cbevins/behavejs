 ...
 * @license MIT
 * 
 * The '/fuel/bed/...' DagNodes define the overall fuel bed characteristics, which are
 * primarily derived from the '/fuel/dead/...' and '/fuel/live/...' characteristics
 * per Rothermel (1972) and BehavePlus V6.
 * 
 * Naming Conventions
 * '/fuel/bed/...' DagNodes are for the combined contribution of the '/fuel/dead' and '/fuel/live' fuel arrays
 ...
 * @param {string} modPrefix String prefixed to each DagNode.key
 * @returns Array of DagNode instances.
 */
export function createDagNodes(modPrefix='') {
    ...
    // Customize node keys with module prefix
    for(let def of defs) {
        def.key = modPrefix + def.key
        // Customize node supplier keys with appropriate preixes
        for (let i=0; i<def.suppliers.length; i++) {
            if (def.suppliers[i].startsWith('/fuel/')) {
                def.suppliers[i] = modPrefix + def.suppliers[i]
            }
        }
    }
