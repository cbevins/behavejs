export class P {
    static bed1 = 'site/surface/primary/bed/'
    static fire1 = 'site/surface/primary/fire/'
    static canopy = 'site/canopy/'
    static windDir = 'site/weather/wind/direction/'
    static windReduct = 'site/weather/wind/reduction/'
    static windSpeed = 'site/weather/wind/speed/'
}
export class C {
    static windSpeedInput = 'config/wind/speed/input/'
}

export class L {
    static at10m = 'at 10-m'
    static at20ft = 'at 20-ft'
    static atMidflame = 'at midflame height'
    static wsrf = 'factor at midflame ht'
}

export class U {
    static canopyHt = 'canopy height, length'
    static factor = 'factor'
    static fraction = 'fraction'
    static fuelDepth = 'fuel depth'
    static fuelLoad = 'ovendry fuel load'
    static heat = 'heat of combustion'
    static hpua = 'heat per unit area'
    static ratio = 'ratio'
    static windSpeed = 'wind speed'
    static yesno = 'yes or no'
}