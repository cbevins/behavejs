...
function log(msg) {
    ...
    for (let i=0; i<dag.topoLevels.length; i++) {
        for(let node of dag.topoLevels[i]) {
            ...
            const dirty = node.tmp ? 'dirty' : 'clean'
            str += `${(''+i).padStart(3)} ${node.key.padEnd(8)} ${method} ${node.status} ${dirty} ${node.value}\n`
        }
    }
    str += `Nodes: ${sel.length} selected, ${clnt.length} client, ${req.length} req, ${kon.length} constant`
    console.log(str)
}
...
dag.init()
log('1: After Dag.init() all nodes should be ignored and clean')

// Require B as an output
dag.select(b)
log('2: After select(b), only B is selected & dirty and rest still ignored & clean')
...
// Update values
dag.update()
log('4: After update: updater() called for for C=>4, D=>8, and E=>12')
