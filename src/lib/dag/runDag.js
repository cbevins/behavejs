...
function log(msg) {
    let str =  `${msg}\nNodes in Kahn Sort Order:\nLvl NodeKey  Type  Dirty Value\n`
    for (let i=0; i<dag.topoLevels.length; i++) {
        for(let node of dag.topoLevels[i]) {
            let method = 'input'
            if (node.updater) {
                method = (node.updater===dag.constant ? "const" : 'updat')
            }
            const dirty = node.tmp ? 'dirty' : 'clean'
            str += `${(''+i).padStart(3)} ${node.key.padEnd(8)} ${method} ${dirty} ${node.value}\n`
        }
    }
    console.log(str)
}
